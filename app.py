...
import shutil
import time
from utils.db_client import init_database, clear_database, drop_and_recreate_tables
...
def super_reset():
    ...
    try:
        ...
        success_db = drop_and_recreate_tables()
        if success_db:
    ...
    **Текущий режим:** Демо-версия
    ...
    if st.button("🔍 Проверить подключение к ClickHouse Cloud"):
        ...
        if result['success']:
            ...
            count_uploads = execute_query('SELECT count(*) FROM video_uploads')
            count_analysis = execute_query('SELECT count(*) FROM video_analysis')
            
            if count_uploads['success'] and count_analysis['success']:
                st.write("**Статистика данных:**")
                st.write(f"- Видео загружено: {count_uploads['data'].strip()}")
                st.write(f"- Анализов выполнено: {count_analysis['data'].strip()}")
        else:
...
with col2:
    ...
    st.write("Удаляет и пересоздает таблицы БД (если обычный не помогает)")
